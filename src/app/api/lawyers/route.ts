...
import { IUserModel, ILawyerModel } from '@/types/models';
...
// Interface for the Populated Lawyer result
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (maxFee) query['fees.perHour'] = { $lte: parseFloat(maxFee) };

    // ✅ Cast result to PopulatedLawyer[] safely
    const lawyers = (await Lawyer.find(query)
      .populate('userId', 'firstName lastName email profileImage')
      .sort({ averageRating: -1, totalReviews: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean()) as PopulatedLawyer[];

    let filteredLawyers = lawyers;

    // ✅ Search filter by lawyer name (case-insensitive)
    if (search) {
...
// ✅ POST - Create lawyer profile (during onboarding)
export async function POST(req: NextRequest) {
  try {
    ...
    return createSuccessResponse(lawyer, 'Lawyer profile created successfully', 201);
  } catch (error) {
    return handleApiError(error);
  }
}
