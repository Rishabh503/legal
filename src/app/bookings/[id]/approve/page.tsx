...
  const [statusMessage, setStatusMessage] = useState<{ type: 'success' | 'error', message: string } | null>(null); // State for visual feedback
  
...
  });
    ...
    // FIX 1: Wrap fetchBooking in useCallback to stabilize the function reference
...
      setLoading(false);
    }
    // Dependencies for useCallback: Only 'params.id' is needed if it's the only external prop/state used inside
  }, [params.id]); 

    // FIX 2: Include fetchBooking in the useEffect dependency array
  useEffect(() => {
    fetchBooking();
  }, [fetchBooking]); // <-- FIX: Use fetchBooking as the dependency
...
      if (data.success) {
        // FIX: Replace alert() with visual status message
        setStatusMessage({ type: 'success', message: 'Booking approved successfully!' });
        setTimeout(() => router.push(`/bookings/${params.id}`), 1500); // Redirect after a short delay
      } else {
        // FIX: Replace alert() with visual status message
        setStatusMessage({ type: 'error', message: data.message || 'Failed to approve booking' });
      }
    } catch (error) {
      console.error('Error approving booking:', error);
      // FIX: Replace alert() with visual status message
      setStatusMessage({ type: 'error', message: 'An unexpected error occurred during approval.' });
    } finally {
      setSubmitting(false);
    }
  };
...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {StatusAlert} {/* Display status alert here */}
